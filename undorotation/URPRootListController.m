#include "URPRootListController.h"

@implementation URPRootListController

- (NSArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"undorotation" target:self] retain];
	}
...
-(void)email{
	if([MFMailComposeViewController canSendMail]){
		...
		[email setSubject:@"UndoRotation Support"];
		[email setToRecipients:[NSArray arrayWithObjects:@"deeppwnage@yahoo.com", nil]];
		[email addAttachmentData:[NSData dataWithContentsOfFile:@"/var/mobile/Library/Preferences/com.dgh0st.undorotation.plist"] mimeType:@"application/xml" fileName:@"Prefs.plist"];
